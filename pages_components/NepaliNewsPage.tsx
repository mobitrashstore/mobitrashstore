                        ...
                        {viewMode === 'reader' && pageContent ? (
                            <div className="max-w-[95vw] lg:max-w-6xl mx-auto p-4 md:p-12 w-full animate-fade-in-up">
                                ...
                                <div className="flex items-center gap-4 mb-10 text-xs font-black text-slate-400 uppercase tracking-[0.2em] border-y border-slate-100 py-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <NewspaperIcon className="w-4 h-4 text-emerald-500" />
                                        <span className="text-slate-900">{activeItem.source}</span>
                                    </div>
                                </div>
                                <div className="prose prose-slate prose-xl lg:prose-2xl max-w-none news-content-viewer overflow-x-hidden selection:bg-emerald-100" dangerouslySetInnerHTML={{ __html: pageContent }}></div>
                            </div>
                        ) : (
                            <div className="w-full h-full flex flex-col">
                                <div className="p-2 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                    ...
                                    <button
                                        ...
                                        className="text-[10px] font-black text-white bg-emerald-500 px-3 py-1 rounded-full uppercase"
                                    >
                                        Open in Browser
                                    </button>
                                </div>
            ...
            <div className="pt-4 px-0">
                ...
                <div className="bg-white py-4 shadow-sm mb-6 border-b border-slate-100 overflow-hidden">
                    <div className="flex gap-5 overflow-x-auto px-6 scrollbar-hide snap-x">
                        ...
                        >
                            <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center bg-slate-50 ${!filterSourceId ? 'border-emerald-500 shadow-lg' : 'border-slate-100'}`}>
                                <NewspaperIcon className="w-8 h-8 text-emerald-500" />
                            </div>
                        ...
                        {sources.map(source => (
                            ...
                            >
                                <div className={`w-16 h-16 rounded-full border-2 overflow-hidden bg-white flex items-center justify-center p-1 cursor-pointer ${filterSourceId === source.id ? 'border-emerald-500 shadow-lg' : 'border-slate-100'}`}>
                    ...
                    {categories.map(cat => {
                        ...
                        return (
                            <div key={cat.id} className="animate-fade-in-up">
                                {/* Section Header */}
                                <div className="flex items-center justify-between mb-4 border-b-2 border-emerald-500/10 pb-2">
                                    <h2 className="text-2xl font-black text-slate-900 tracking-tighter flex items-center gap-2">
                                        <div className="w-2 h-7 bg-emerald-500 rounded-full shadow-sm shadow-emerald-200"></div>
                                        {cat.label}
                                    </h2>
                                    <button className="text-[11px] font-black text-slate-400 bg-slate-100 px-3 py-1 rounded-full uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all">सबै हेर्नुहोस्</button>
                                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex-grow flex flex-col justify-between py-1 min-w-0">
                                                    <h3 className="text-[15px] font-black text-slate-900 line-clamp-3 leading-[1.3] group-hover:text-emerald-600 transition-colors tracking-tight">
                                                        {item.title}
                                                    </h3>
                                                    <div className="flex items-center justify-between mt-auto">
                                                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter truncate max-w-[100px] bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100">{item.source}</span>
