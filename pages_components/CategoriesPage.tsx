                ...
                {categories.length > 0 ? (
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                        {categories.map(cat => (
                            ...
                            >
                                <div className="w-full aspect-square bg-gray-50 rounded-xl p-4 flex items-center justify-center border border-gray-100 shadow-sm group-hover:border-[#059669] group-hover:shadow-md transition-all duration-200">
                                    <img src={cat.imageUrl} alt={cat.name} className="w-full h-full object-contain" />
                                </div>
                                <span className="mt-2 text-sm font-medium text-gray-700 group-hover:text-[#059669] text-center leading-tight">{cat.name}</span>
                            </button>
                        ))}
                    </div>
                ) : (
